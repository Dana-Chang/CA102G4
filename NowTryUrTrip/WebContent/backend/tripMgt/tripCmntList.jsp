...
<%@ page import="java.io.*"%>
<%@ page import="com.tripCmnt.model.*"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%
	TripCmntService tripCmntSvc = new TripCmntService();
	List<TripCmntVO> list = tripCmntSvc.getAll();
...
<!-- --------------------內容本體------------------------- -->
	...
	<!-- 麵包屑 -->
		<div class="header">
			...
			<ul class="breadcrumb">
				<li>
					<a href="listAllTrip.jsp">行程列表</a>
				</li>
				<li class="active">行程評論列表</li>
			</ul>
		</div>
	<!-- 錯誤回報區 -->
		<div>
		...
		<table border='1' bordercolor='#CCCCFF' width='800'>
			...
			<c:forEach var="tripCmntVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
				<tr align='center' valign='middle'>
					<td>${tripCmntVO.tripCmntNo}</td>
					<td>${tripCmntVO.memId}</td>
					<td>${tripCmntVO.tripNo}</td>
					<td>${tripCmntVO.tripCmntContent}</td>
					<td><fmt:formatDate value="${tripCmntVO.tripCmntTime}" pattern="yyyy/MM/dd HH:mm:ss" /></td>
					<td>${tripCmntVO.tripRate}</td>
					<td>${isHdlMap.get(tripCmntVO.tripCmntIsHdl)}</td>
					<td>${isBlockedMap.get(tripCmntVO.tripCmntIsBlocked)}</td>
					<td>${tripCmntVO.tripCmntBlockedReason}</td>
					<td>
						<FORM METHOD="post" ACTION="backTripCmnt.do">
							<button type="submit">
								<i class="fa fa-pencil"></i>
							</button>
							<input type="hidden" name="tripCmntNo" value="${tripCmntVO.tripCmntNo}">
							<input type="hidden" name="action" value="getOne_For_Update">
						</FORM>
					</td>
					<td>
						<button href="#myModal" data-toggle="modal" onclick="deleteFunction(${tripCmntVO.tripCmntNo})">
