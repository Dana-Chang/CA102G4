...
<%@ page import="java.io.*"%>
<%@ page import="com.tripCmntRpt.model.*"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%
	TripCmntRptService tripCmntRptSvc = new TripCmntRptService();
	List<TripCmntRptVO> list = tripCmntRptSvc.getAllSorted();
	...
	<!-- 麵包屑 -->
		<div class="header">
			...
			<ul class="breadcrumb">
				...
				<li>
					<a href="listAllSpot.jsp">行程列表</a>
				</li>
				<li class="active">行程評論檢舉處理</li>
			</ul>
		</div>
	<!-- 錯誤回報區 -->
		<div>
...
<%-- 		<jsp:useBean id="tripCmntRptSvc" scope="page" class="com.spotCmntRpt.model.TripCmntRptService" /> --%>
<!-- 		<li> -->
<!-- 			<FORM METHOD="post" -->
...
<!-- 		</li> -->
		<table border='1' bordercolor='#CCCCFF' width='800'>
			...
			<jsp:useBean id="tripCmntSvc" scope="page" class="com.tripCmnt.model.TripCmntService" />
			<c:forEach var="tripCmntRptVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
				<tr align='center' valign='middle'>
					<td>${tripCmntRptVO.tripCmntRptNo}</td>
					<td>${tripCmntRptVO.memId}</td>
					<td>${tripCmntRptVO.tripCmntNo}</td>
					<td>${tripCmntSvc.getOneTripCmnt(tripCmntRptVO.tripCmntNo).tripCmntContent}</td>
					<td>${tripCmntRptVO.rptReason}</td>
					<td><fmt:formatDate value="${tripCmntRptVO.rptTime}" pattern="yyyy/MM/dd HH:mm:ss" /></td>
					<td>${isBlockedMap.get(tripCmntSvc.getOneTripCmnt(tripCmntRptVO.tripCmntNo).tripCmntIsBlocked)}</td>
					<td>${isHdlMap.get(tripCmntSvc.getOneTripCmnt(tripCmntRptVO.tripCmntNo).tripCmntIsHdl)}</td>
					<td>${tripCmntSvc.getOneTripCmnt(tripCmntRptVO.tripCmntNo).tripCmntBlockedReason}</td>
					<td width=250px>
                    <!-- **********屏蔽********** -->
						<form class="form-inline" style="margin-top: 0px;float:left" action="backTripCmntRpt.do">
							...
							<input type="hidden" name="tripCmntRptNo" value="${tripCmntRptVO.tripCmntRptNo}">
...
<!-- 					<td> -->
<%-- 						<button href="#myModal" data-toggle="modal" onclick="deleteFunction(${spotCmntVO.tripCmntNo})"> --%>
...
<!-- 							<input type="hidden" name="tripCmntNo" value=""> -->
...
// 		$(".modal-footer input[name='tripCmntNo']").val(e);
// 	}
<!-- </script> -->