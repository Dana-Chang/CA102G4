		...
		<FORM METHOD="post" ACTION="backSpotCmnt.do" name="form1" enctype="multipart/form-data">
			<table border="0">
				<tr>
					<td>評論者ID:</td>
					<td>
						<input type="TEXT" name="memId" value="<%=(spotCmntVO == null) ? 1 : spotCmntVO.getMemId()%>" />
					</td>
				</tr>
				<tr>
					<td>景點編號:</td>
					<td>
						<input type="TEXT" name="spotNo" value="<%=(spotCmntVO == null) ? 1 : spotCmntVO.getSpotNo()%>" />
					</td>
				</tr>
				<tr>
					<td>評論內容:</td>
					<td>
						<input type="text" name="cmnt" value="<%=(spotCmntVO == null) ? "安安你好這是測試" : spotCmntVO.getCmnt()%>" />
					</td>
				</tr>
				<tr>
					<td>評論時間:</td>
					<td>
						<%=sdf.format(new java.util.Date().getTime())%>
					</td>
				</tr>
				<tr>
					<td>景點給分:</td>
					<td>
						<input type="text" name="rate" value="<%=(spotCmntVO == null) ? 9 : spotCmntVO.getRate()%>" />
					</td>
				</tr>
				<tr>
					<td>是否檢查:</td>
					<td>
						<input type="text" name="isChecked" value="<%=(spotCmntVO == null) ? 0 : spotCmntVO.getIsChecked()%>" />
					</td>
				</tr>
				<tr>
					<td>是否屏蔽:</td>
					<td>
						<input type="TEXT" name="isBlocked" value="<%=(spotCmntVO == null) ? 0 : spotCmntVO.getIsBlocked()%>" />
					</td>
				</tr>
				<tr>
					<td>屏蔽原因:</td>
					<td>
						<input type="TEXT" name="BlockedReason" value="<%=(spotCmntVO == null) ? "" : spotCmntVO.getBlockedReason()%>" />
					</td>
				</tr>
			</table>
			<br>
