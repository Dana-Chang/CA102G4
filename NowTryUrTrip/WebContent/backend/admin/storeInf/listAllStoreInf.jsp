...
<jsp:useBean id="storeInfSvc" scope="page"
	class="com.storeInf.model.StoreInfService" />
<%
	StoreInfVO storeInfVO = (StoreInfVO) session.getAttribute("storeInfVO");
	ManagerVO loginManagerVO = (ManagerVO) session.getAttribute("loginManagerVO");
%>
<%
	StoreInfService StoreInfSvc = new StoreInfService();
	List<StoreInfVO> list = StoreInfSvc.getAll();
	...
	<div class="content">
		...
		<div class="main-content">
			...
			<table border='1' bordercolor='#CCCCFF' width='800'>
				...
				<c:forEach var="storeInfVO" items="${list}" begin="<%=pageIndex%>"
					end="<%=pageIndex+rowsPerPage-1%>">
					<tr align='center' valign='middle'>
						<td>${storeInfVO.memId}</td>
						<td>${storeInfVO.uniformNum}</td>
						<td>${storeInfVO.principal}</td>
						<td>${storeInfVO.storCall}</td>
						<td>${storeInfVO.storAdd}</td>
						<td>${storeInfVO.postalCode}</td>
						<td>${storeInfVO.category}</td>
						<td>
							<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/Manager/mgr.do">
								<input type="submit" value="修改"> <input type="hidden" name="mgrId" value="${storeInfVO.memId}"> 
